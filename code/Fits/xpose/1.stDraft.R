...
SaveXpose <- function(filename, plotname, folder = "Fits/XposePlots/"){
    full_path <- paste0(folder, filename)
    ggsave(full_path, plot = plotname, width = 13, height = 6, dpi = "retina")
}

names(results1Comp)
myfit <- results1Comp[["1Comp_DV_100_Init_1"]]
myfit <- resultsBW[["BW_ETA_ALL_DV_100_Init_1"]]
...
res_vs_idv(xpdb, res="CWRES")      # Residuals vs IDV
res_vs_pred(xpdb, res="CWRES")     # Residuals vs model prediction
absval_res_vs_idv(xpdb, res="CWRES") # Absolute residuals vs IDV
absval_res_vs_pred(xpdb, res="CWRES") # Absolute residuals vs model prediction

# Grouped Plots
dv_vs_idv(xpdb, group="ID")        # Dependent variable vs IDV
ipred_vs_idv(xpdb, group="ID")     # Individual predictions vs IDV
pred_vs_idv(xpdb, group="ID")      # Model predictions vs IDV
dv_preds_vs_idv(xpdb)              # Dependent variable, predictions vs IDV
