...
for (i in c(10, 25, 50, 100)) {
  variable_name <- paste0("AllETA_CP_", i)
  assign(variable_name, nlmixr(two.cmt.depot.all, subsetTrue[subsetTrue$ID >= 1 & subsetTrue$ID <= i, ], est = "focei"))
}


# DV
for (i in c(10, 25, 50, 100)) {
  variable_name <- paste0("AllETA_DV_", i)
  assign(variable_name, nlmixr(two.cmt.depot.all, subsetNoise[subsetNoise$ID >= 1 & subsetNoise$ID <= i, ], est = "focei"))
}