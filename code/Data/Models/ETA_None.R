...
initials_list <- list(
  list(tka_init = 0.0025, tq_init = 0.304, tcl_init = 0.034, tvc_init = 3.59, tvp_init = 4.1, tf_init = 0.847, add.sd_init = 0.2)
  #list(tka_init = 1.1, tq_init = 3.0, tcl_init = 2.5, tvc_init = 25, tvp_init = 55, tf_init = 1.5),
  #list(tka_init = 1.5, tq_init = 2.5, tcl_init = 1.9, tvc_init = 35, tvp_init = 65, tf_init = 2.0)
)
...
# MSE_None df
MSE_None_values <- numeric(length = length(results))

for (i in seq_along(results)) {
  MSE_None_values[i] <- 1 / length(results[[i]]$IRES) * sum((results[[i]]$IRES)^2)
}

MSE_None <- data.frame(MSE_None = MSE_None_values)
rownames(MSE_None) <- names(results)
