...
for (i in Amount_Of_Subjects) {
  for (j in 1:length(initials_list)) {
    ...
    # Fit for DV
    results_BW_Eta_V[[key_dv]] <- nlmixr(
      do.call(two.cmt.depot.V2.V3.BW, initials_list[[j]]),
...
# Initialize an empty data frame for summarizing results_BW_Eta_V
statSummary <- data.frame()

# Extract objDf for each model and concatenate results_BW_Eta_V into statSummary
for (result_name in names(results_BW_Eta_V)) {
  if (!is.null(results_BW_Eta_V[[result_name]]$objDf)) {
    statSummary <- bind_rows(statSummary, results_BW_Eta_V[[result_name]]$objDf)
  }
}

# Assign new row names based on the results_BW_Eta_V
rownames(statSummary) <- names(results_BW_Eta_V)
...
# MSE_V_BW df
MSE_V_BW_values <- numeric(length = length(results_BW_Eta_V))

for (i in seq_along(results_BW_Eta_V)) {
  MSE_V_BW_values[i] <- 1 / length(results_BW_Eta_V[[i]]$IRES) * sum((results_BW_Eta_V[[i]]$IRES)^2)
}

MSE_V_BW <- data.frame(MSE_V_BW = MSE_V_BW_values)
rownames(MSE_V_BW) <- names(results_BW_Eta_V)
...
for (result_name in names(results_BW_Eta_V)) {
  if (!is.null(coef(results_BW_Eta_V[[result_name]])[[1]])) {
    coefSummary <- bind_rows(coefSummary, coef(results_BW_Eta_V[[result_name]])[[1]])
  }
}

rownames(coefSummary) <- names(results_BW_Eta_V)
...
for (result_name in names(results_BW_Eta_V)) {
  if (!is.null(results_BW_Eta_V[[result_name]]$parFixed)) {
    
    # Create a new data frame for the current result_name
    current_data_frame <- results_BW_Eta_V[[result_name]]$parFixed
